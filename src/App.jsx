...
const HOME_STORAGE_KEY = "tamsui-kindergarten-picker-homes";
const HOME_PRESET_QUERY_KEY = "home";
const JOEY_PRESET = "joey";

const presetHomeInputs = {
  homeA: {
...
const shouldUsePresetHome = () => {
  const searchParams = new URLSearchParams(window.location.search);
  if (searchParams.get(HOME_PRESET_QUERY_KEY) === JOEY_PRESET) return true;

  const hashQuery = window.location.hash.split("?")[1] ?? "";
  const hashParams = new URLSearchParams(hashQuery);
  return hashParams.get(HOME_PRESET_QUERY_KEY) === JOEY_PRESET;
};
...
const resolveHomes = (customHomes) =>
  Object.fromEntries(
    Object.entries(presetHomeInputs).map(([key, defaultHome]) => {
...
function useCustomHomes() {
  const [customHomes, setCustomHomes] = useState(() => {
    if (shouldUsePresetHome()) {
      return presetHomeInputs;
    }
    ...
    try {
      ...
      return matchesHomeInputs(hydratedHomes, presetHomeInputs) ? emptyHomeInputs : hydratedHomes;
    } catch {
  ...
  return (
    <header className="site-header">
      ...
      <nav>
        ...
        <a href="https://github.com/bluetch/ntpc-kindergarten-2026" target="_blank" rel="noreferrer" title="原中永和版本（本站 fork 自此）">
          原版
        </a>
      </nav>
    </header>
  ...
  return (
    ...
    >
      ...
      <div className="card-title">
        ...
        {Array.isArray(school.punishRecord) && school.punishRecord.length > 0 && (
